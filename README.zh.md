...
你每天都在使用 coding agent，但你有没有想过它们到底是怎么工作的？

<p align="center">
  <img src="docs/demo.gif" alt="mini-claw-code 在终端中运行" width="700">
</p>

比你想象的要简单。去掉 UI、流式传输、模型路由——每个 coding agent 的核心就是这个循环：
...
每个 coding agent——包括你将要构建的——都基于这个循环运行：
...
| 5 | `SimpleAgent` | 循环 —— 将 single_turn 扩展为真正的 agent |
| 6 | `OpenRouterProvider` | HTTP 调用真实 LLM（OpenAI 兼容 API）|
| 7 | CLI 聊天应用 | 用约 15 行代码将所有组件串联起来 |

**第二部分 —— 奇点时刻**（你的 agent 开始自己写代码了）

| 章 | 主题 | 新增能力 |
|----|------|---------|
| 8 | 奇点时刻 | 你的 agent 可以编辑自己的源代码 |
...
本 agent 拥有**不受限的 Shell 访问权限**。`BashTool` 会将 LLM 生成的命令直接传递给 `bash -c`，没有沙箱、过滤或超时机制。`ReadTool` 和 `WriteTool` 可以访问你用户账户能访问的任何文件。这对学习项目来说是有意为之，但请注意：

- **不要对不受信任的提示词或文件运行此 agent**（通过文件内容的提示词注入可以执行任意命令）。
