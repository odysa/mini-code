...
1. **计划阶段** —— agent 使用只读工具（`read`、`bash` 和 `ask_user`）探索代码库。它不能写入、编辑或修改任何内容。它返回一个描述其意图的计划。
2. **执行阶段** —— 用户审查并批准计划后，agent 再次运行，此时所有工具都可用。
...
关键洞察：**同一个 agent 循环适用于两个阶段**。唯一的区别是*哪些工具可用*。
...
- **`plan()`** —— 注入系统提示词，仅使用只读工具和 `exit_plan` 运行 agent 循环。
- **`execute()`** —— 使用所有工具运行 agent 循环。
...
- **`PlanAgent`** 通过一个共享的 agent 循环，将计划（只读）与执行（所有工具）分离。
...
- **调用方驱动的批准**：agent 不管理批准——调用方将批准/拒绝作为 `User` 消息推入并调用相应的阶段。
