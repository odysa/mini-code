...
Open `mini-code-starter/src/types.rs` and look at the `Tool` trait:
...
Open `mini-code-starter/src/tools/read.rs`. The struct, `Default` impl, and
...
```bash
cargo test -p mini-code-starter ch2
```
